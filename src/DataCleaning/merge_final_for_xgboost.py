...
# Dirs for properties_final
EXTRACTED_DIR = "data/llm_extracted_final"
OUTPUT_FILE = "data/xgboost_training_data_final.json"
    ...
    for filename in extracted_files:
        ...
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                # Map features by added property_id
...
def prepare_for_xgboost():
    ...
    try:
        with connection.cursor() as cursor:
            # Fetch base records from the new final table
            print("Fetching base records from properties_final table...")
            cursor.execute("SELECT * FROM properties_final")
            db_records = cursor.fetchall()
            
            print(f"Found {len(db_records)} records in the database. Merging with LLM features...")
            
            for record in db_records:
                prop_id = str(record.get('property_id'))
